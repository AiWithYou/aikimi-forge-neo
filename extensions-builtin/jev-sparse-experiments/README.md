...
速度優先版の層別Jevは、全4モデルで初回のみ・指定間隔（1〜100step）・毎stepを選べます。既定は初回のみです。Krea2では1/3/5/10/25/50/100%、Anima・Qwenでは25/50/75/100%、H3では1/3/5/10%から選びます。構図や細部の違いは許容する設計です。有効な回答を信頼度だけで100%へ置き換えず、回答・信頼度・適用値を記録します。APIエラー時の退避とキャンセルは維持します。
...
H3はFused Turbo・4 steps・res_multistep限定です。49層を一括で判定し、層0は固定5%です。初回のみで1回、2step間隔で2回、毎stepで3回を問い合わせます。既存の標準H3環境に `aikimi-jev-setup.bat --h3` でノードを追加・更新できます。導入済みの旧版も実行環境を停止して更新してください。通常のH3モデルを4 stepsに自動変更する機能ではありません。
