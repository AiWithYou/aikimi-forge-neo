...
**まだ保証していないこと:** Windows上の実GPU生成、16GB VRAMでの完走、速度・音質、日本語歌唱、既存Forge全体とのGPU切替。CPUテストの合格は、これらの検証結果ではありません。
...
停止は現在のブラウザーセッションから投入したYuE2ジョブだけを対象にします。協調停止が進まなければ専用プロセスツリーを終了します。WindowsはJob Object、POSIXは専用プロセスグループと親パイプ監視を使います。GPUの所有権はworkerの終了確認後に返します。Windows固有処理は別途実機テストが必要です。
