...
**このタブは参照の準備と完成画像の合成を行います。画像生成には、別途Qwen Image 2.1対応ComfyUIと[ausbossのOutpaint LoRA・ワークフロー](https://huggingface.co/ausboss/Qwen-Image-2.1-Outpaint-LoRA)が必要です。**

1. **Qwen Outpaint 補助**で元画像を選び、「広げる方向」と各方向の余白を指定します。斜線が新しく生成する範囲、橙枠が元画像です。プレビューで完成寸法を確認できます。
2. **参照PNGを作成 → 参照PNGを保存**。表示されたLoRA名・指示・設定を使ってComfyUIで生成します。参照PNGを`image_1`へ入れ、resolutionは`0`、エンコーダーのlatentを使用します。出力寸法は画面の指定に合わせます。設定メモのJSONはワークフローではありません。
3. 生成画像をこのタブへ戻し、**完成画像を作成 → 完成画像を保存**。境界ぼかしを0にすると、元画像の全画素を保持します。値を増やすと拡張側の内縁をなじませます。

元画像を拡縮せず、指定した方向に32 px単位で余白を足します。各辺256〜4096 px、合計約2 MPまでです。参照の条件を変えた場合は再作成し、今回の参照から生成した画像を選び直してください。[方式・制約・検証記録](../../docs/qwen-image21-community-2026-09-26.md)
