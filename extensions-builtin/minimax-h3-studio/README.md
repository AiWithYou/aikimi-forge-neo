...
**v3.0.0の寸法指定：** 解像度プリセットに加えて幅・高さを直接入力でき、縦横入替も使えます。各辺256〜2048 px・32の倍数です。手入力中は比率プリセットを無効にし、解像度プリセットを選ぶと通常の比率指定へ戻ります。実際の寸法は負荷表示・生成履歴・ComfyUIへの書き出しにも引き継ぎます。大きい寸法や長い動画ではRAM／VRAM負荷が増えるため、既存の生成前チェックが引き続き適用されます。
