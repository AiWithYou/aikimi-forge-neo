...
保持率は残す接続の割合です。重みの削減率ではありません。速度優先版ではJevに、弱い・安定した寄与は低い保持率、通常の寄与は中間、極端に強い寄与は高い保持率を選ぶ目的を伝えます。Anima・Qwenは25/50/75/100%、Krea2は1/3/5/10/25/50/100%、H3は1/3/5/10%が候補です。層別の呼び出しは既定で1回。初回のみでは以後の観測用計算も省きます。Krea2では層の判定頻度を指定間隔・毎stepへ変更でき、タイル配分も初回のみ・拡大段階ごとを選べます。[回数の決め方と制約](krea2-jev.md#api回数の決め方)
