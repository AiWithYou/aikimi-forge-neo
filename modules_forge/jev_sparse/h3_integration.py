...
CHOICES = [("H3比較 Dense · 4 Steps / 計測", "h3_dense"), ("H3固定SLA 5% · 4 Steps / 通信なし", "h3_fixed5"), ("H3固定SLA 10% · 4 Steps / 通信なし", "h3_fixed10"), ("H3 Jev制御SLA · 4 Steps / 外部送信", "h3_jev")]
...
def pack_files():
    ...
    return {"__init__.py": b"from .jev_sparse.h3_node import NODE_CLASS_MAPPINGS, NODE_DISPLAY_NAME_MAPPINGS\n",
            **{"jev_sparse/" + name: (source / name).read_bytes() for name in ("__init__.py", "common.py", "sdk_worker.py", "h3_node.py")}}
...
def verify_runtime(root: Path):
    ...
    if not sparse.is_file() or git_blob(sparse.read_bytes().replace(b"\r\n", b"\n")) != SPARSE_BLOB:
        raise ValueError("H3実験の内部APIが固定版と一致しません。通常のH3環境は変更せず、setup_jev_sparse.py --create-h3-runtime で実験環境を作成してください。")
    target = root / "custom_nodes" / PACK
    for name, expected in pack_files().items():
        path = target / name
        if not path.is_file() or path.is_symlink() or path.read_bytes() != expected:
            raise ValueError("H3実験ノードが未導入か版違いです。setup_jev_sparse.py --h3 --comfy-root <ComfyUI> を実行してください。")
...
def patch_workflow(graph, mode, python=""):
    ...
    conditions = [node for node in graph.values() if node.get("class_type") in {"MiniMaxH3ImageToVideo", "MiniMaxH3ReferenceToVideo"}]
    if len(conditions) != 1:
    ...
    if any("Control" in str(n.get("class_type", "")) or n.get("class_type") in {"BlockSparseAttention", NODE} for n in graph.values()):
    ...
    graph[ident] = {"class_type": NODE, "inputs": {"model": list(model), "mode": mode, "sdk_python": python, "prompt_context": prompt if isinstance(prompt, str) else ""}}
...
def install():
    ...
    def validate_experiment(self):
        ...
        if self.model_variant != "fused_turbo" or self.clip_cache != "off":
            raise ValueError("H3実験はFused Turbo + 4 Steps、CLIPキャッシュOFFに限定しています。")
    ...
    def validate_node_schema(self, schemas):
        ...
        if not isinstance(values, (list, tuple)) or not set(MODES.values()) <= set(values) or tuple(schema.get("output", ())) != ("MODEL",):
            raise ValueError("H3 experiment node mode/output schema mismatch")
    for name, function in zip(required, (validate_experiment, runtime_packs, extra_nodes, apply_workflow, validate_node_schema)):
    ...
    def note(*values):
        ...
        if option.attention not in MODES:
            return old_note(*values)
        cloud_note = "Jevはプロンプトと集約統計を外部APIへ送信します。生画像・音声・重みは送信しません。" if option.attention == "h3_jev" else "この比較モードはクラウド通信しません。"
        return '<div role="status">H3実験：4 Steps / res_multistep限定。' + cloud_note + ' 全50層を実行し、音声・条件の保護はnative SLAに従います。保持率は総計算削減率ではありません。既存のSparse開始位置は使用せず、開始から適用します。12288 tokens未満はdenseです。画質・速度はGPU未検証。ログ：outputs/jev-sparse。選択変更後は実行環境を再起動してください。</div>'
    def state(*values):
        result = old_state(*values)
        try:
            if cls.from_values(values).attention in MODES:
                import gradio as gr
                result = (note(*values), result[1], gr.update(visible=False), gr.update(visible=False), gr.update(visible=False))
        except ValueError:
            pass
        return result
...
def after_component(component, **kwargs):
    if getattr(component, "elem_id", None) == "h3-runtime-path":
        ...
        component.info = "実験用はrepositories/minimax-h3-jev/ComfyUI。標準環境は変更しません。環境切替前にNeoを終了し、再起動してください。"
