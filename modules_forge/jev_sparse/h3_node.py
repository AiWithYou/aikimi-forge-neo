...
class H3Controller:
    def __init__(self, mode, log, client=None, prompt=""):
    ...
    def initialize(self):
        ...
        self._decide({"initialization": True, "next_step": 1, "prompt": self.prompt,
                      "constraints": "First denoising step, no current-run observations and no historical statistics. Layer 0 allows only 5 or 10 percent. All layers run. Exact conditioning KV/audio query protection comes from native SLA. No quota or forced variation; uncertain layers should use 10."}, allowed, 10.0, initial=True)
        self.prompt = ""

    def _decide(self, state, allowed, fallback, initial=False):
        try:
            result = self.client.decide(state, allowed, fallback)
            self.keeps = [result[str(i)] for i in range(50)] if initial else [5.0] + [result[str(i)] for i in range(1, 50)]
            reason = "jev"
        ...
        self.log.write("decision", next_step=self.step + (1 if initial else 2), source=reason, keep_percent=self.keeps[:], api_calls=self.client.calls, api_wait_seconds=self.client.wait_seconds)
    ...
    def observe(self, layer, before, after):
        ...
        for kind in ("audio", "video"):
            ...
            drift = delta.new_tensor(-1.0) if prev is None or prev.shape != delta.shape else (delta - prev).norm() / prev.norm().clamp_min(1e-8)
    ...
    def done(self, index):
        ...
        if self.mode == "jev" and index < 3 and not self.disabled:
            import torch
            ...
            if not all(math.isfinite(v) for row in rows for v in row):
                ...
                state = {"step_measured": index + 1, "next_step": index + 2,
                         "constraints": "Layer 0 is fixed at 5 percent and is not a question. Choose 1,3,5,10 for layers 1..49. All blocks run. Exact conditioning KV/audio query rows are protected by native SLA; downstream audio is not guaranteed identical. Residual magnitude, peer rank and drift are uncalibrated proxies, not measured quality or sparse error. No forced variation or quota.",
                         "blocks": {str(i): {kind: {"residual_relative_l2": rows[i][j], "rank": (ranks[kind].index(i) + 1) / 50, "cross_step_change": None if rows[i][j + 1] < 0 else rows[i][j + 1]} for kind, j in (("audio", 0), ("video", 2))} for i in range(50)},
...
class AikimiH3SparseExperiment:
    @classmethod
    def INPUT_TYPES(cls):
        return {"required": {"model": ("MODEL",), "mode": (["dense", "fixed5", "fixed10", "jev"],), "sdk_python": ("STRING", {"default": ""}), "prompt_context": ("STRING", {"default": "", "multiline": True})}}
    RETURN_TYPES = ("MODEL",)
    FUNCTION = "patch"
    ...
    def patch(self, model, mode, sdk_python="", prompt_context=""):
        ...
        if mode != "dense":
            ...
            m.add_callback_with_key(pe.CallbacksMP.ON_PREPARE_STATE, "aikimi_jev", lambda mp, t, opts: install_override(p, opts["transformer_options"]))
        ...
        def make(block, index):
            ...
            def call(args, extra):
                ...
                eligible = mode != "dense" and h3_eligible(block.attn, args["img"], args["rope_freqs"], args["transformer_options"], p, index)
                c.actual[str(index)] = "native_sla_producer" if eligible else "dense" if mode == "dense" else "native_producer_not_used"
                observe = mode == "jev" and c.step < 3 and not c.disabled
        ...
        def wrapper(executor, model_wrap, sigmas, extra_args, callback, noise, latent_image=None, denoise_mask=None, disable_pbar=False):
            if len(sigmas) != 5 or getattr(executor.class_obj.sampler_function, "__name__", "") != "sample_res_multistep":
            ...
            log = RunLog(root, "h3", {"mode": mode, "steps": 4, "sampler": "res_multistep", "timing_scope": "sampling_only", "min_tokens": 12288, "extra_tokens": 0, "sigmas": sigmas.detach().cpu().tolist()}, prompt_context)
            client = None
            c = None
            status = "failed"
            try:
                client = JevClient(Path(sdk_python), 20, cancelled) if mode == "jev" else None
                c = H3Controller(mode, log, client, prompt_context)
                p.controller = c
                c.initialize()
                ...
                log.finish(status, api_calls=client.calls if client else 0, api_wait_seconds=client.wait_seconds if client else 0)
                p.controller = None
                p.reset()
                if c is not None:
                    c.previous.clear()
