"""Isolated typesafe-sdk 0.7.0 worker. One bounded JSON line per decision."""

from __future__ import annotations

import importlib.metadata
import json
import math
import sys
from contextlib import ExitStack, redirect_stdout


def questions_for(payload, Choice):
    state, allowed = payload["state"], payload["allowed"]
    if not isinstance(state, dict) or not isinstance(allowed, dict) or not 1 <= len(allowed) <= 256:
        raise ValueError("Invalid request")
    timeout = float(payload["timeout"])
    if isinstance(payload["timeout"], bool) or not math.isfinite(timeout) or not 0.5 <= timeout <= 20:
        raise ValueError("Invalid timeout")
    if any(
        not isinstance(layer, str)
        or not isinstance(choices, list)
        or not choices
        or any(isinstance(v, bool) or not isinstance(v, (int, float)) or not math.isfinite(v) for v in choices)
        for layer, choices in allowed.items()
    ):
        raise ValueError("Invalid choices")
    tile_steps = state.get("decision_kind") == "tile_steps"
    questions = {
        layer: Choice(
            instructions=(
                f"Choose diffusion STEPS for tile detail group {layer}. Zero skips diffusion and retains the enlarged input. Positive choices regenerate that group for the selected step count. Follow state.constraints; compare measured detail statistics across groups. Prefer less compute for weak detail, more for strong detail. Do not invent faces, text, or quality measurements, or force quotas. Return only a supplied choice."
                if tile_steps
                else f"Choose attention KEEP percentage for layer {layer} to trade compute cost against estimated visual importance. Follow state.constraints and compare the supplied measurements with peer layers. Pixel-identical reproduction is NOT the objective; visually good, prompt-consistent generation with less compute is. Use lower keep for relatively weak or stable contributions, middle keep for typical or ambiguous contributions, and maximum keep for unusually strong or rapidly changing contributions. Do not default to maximum solely because these are proxy measurements. Do not invent image content, measured quality scores, or force a quota or variation. Return only a supplied choice."
            ),
            criteria={
                str(float(keep)): (
                    "Skip diffusion; retain enlarged input"
                    if tile_steps and keep == 0
                    else f"Generate using {int(keep)} diffusion steps"
                    if tile_steps
                    else f"Keep {keep}% of eligible key blocks; larger values are more conservative"
                )
                for keep in choices
            },
        )
        for layer, choices in allowed.items()
    }
    return state, questions, timeout


def main() -> int:
    protocol = sys.stdout
    try:
        if importlib.metadata.version("typesafe-sdk") != "0.7.0":
            raise ValueError("Unexpected SDK version")
        # stdout belongs exclusively to the protocol, including during SDK import.
        with redirect_stdout(sys.stderr):
            from typesafe_sdk import Choice, RetryPolicy, TypeSafeClient
        with redirect_stdout(sys.stderr), ExitStack() as resources:
            client = None
            timeout_setting = None
            while True:
                raw = sys.stdin.buffer.readline(1024 * 1024 + 2)
                if not raw:
                    break
                if len(raw) > 1024 * 1024 or not raw.endswith(b"\n"):
                    raise ValueError("Input limit")
                state, questions, timeout = questions_for(json.loads(raw), Choice)
                with redirect_stdout(sys.stderr):
                    if client is None:
                        client = resources.enter_context(
                            TypeSafeClient(
                                model="jev-1.13.0",
                                retry=RetryPolicy(max_retries=0),
                                timeout=timeout,
                                base_url="https://api.typesafe.ai",
                            )
                        )
                        timeout_setting = timeout
                    elif timeout != timeout_setting:
                        raise ValueError("Session timeout changed")
                    response = client.system_one(state=state, questions=questions)
                answer = {
                    "decisions": {
                        str(layer): {"choice": choice.choice, "confidence": choice.confidence}
                        for layer, choice in response.choices.items()
                    }
                }
                encoded = json.dumps(answer, allow_nan=False) + "\n"
                if len(encoded.encode("utf-8")) > 1024 * 1024:
                    raise ValueError("Output limit")
                protocol.write(encoded)
                protocol.flush()
        return 0
    except Exception:
        # HTTP errors may contain prompts or credentials. Never print the exception.
        protocol.write('{"error":"sdk_failure"}\n')
        protocol.flush()
        return 1


if __name__ == "__main__":
    raise SystemExit(main())
