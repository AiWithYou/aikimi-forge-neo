...
class JevClient:
    ...
    def __init__(
        self, python: Path, timeout: float = 5.0, cancelled: Callable[[], bool] | None = None, *, environment=None
    ):
        if (
    ...
    def decide(self, state: dict, allowed: Mapping[str, tuple[float, ...]], fallback: float) -> dict[str, float]:
        ...
        proc = None
        try:
            proc = subprocess.Popen(  # noqa: S603 -- resolved interpreter and fixed local worker, no shell
