...
from modules_forge.jev_sparse import common, qwen21 as q, qwen21_integration as integration
...
def prepare(attn, hidden, rope, cache, mode, write_slice):
    ...
    if rope is not None:
        def rotate(x):
            return torch.view_as_real(torch.view_as_complex(x.float().reshape(*x.shape[:-1], -1, 2)) * rope.unsqueeze(1)).flatten(3).to(x.dtype)
...
class DefaultProcessor:
    ...
    def __call__(self, attn, hidden_states, attention_mask=None, rotary_emb=None, layer_cache=None, kv_cache_mode=None, cache_write_slice=None, segments=None, key_valid=None):
        ...
        def sdpa(start, end, key_end, mask=None):
            return F.scaled_dot_product_attention(query[:, start:end].transpose(1, 2), key[:, :key_end].transpose(1, 2), value[:, :key_end].transpose(1, 2), attn_mask=mask).transpose(1, 2)
        if segments is None:
            ...
            for start, end, text in segments:
                mask = None
                if text:
                    mask = torch.cat((torch.ones(end-start, start, dtype=torch.bool), torch.ones(end-start, end-start, dtype=torch.bool).tril()), 1)[None, None]
...
class QwenImage21Transformer2DModel(torch.nn.Module):
    def __init__(self, layers=3):
        super().__init__()
        self.transformer_blocks = torch.nn.ModuleList([torch.nn.ModuleDict({"attn": Attention()}) for _ in range(layers)])
    ...
    def forward(self, x, mode, mask=None, rope=None):
        for i, block in enumerate(self.transformer_blocks):
            x = block.attn(x, attention_mask=mask if mode == "cached" else None, rotary_emb=rope, layer_cache=self.caches[i], kv_cache_mode=mode, cache_write_slice=slice(0, 13) if mode == "extract" else None, segments=[(0, 5, True), (5, 13, False)] if mode == "extract" else None, key_valid=mask[:, 0, 0] if mask is not None and mode == "extract" else None)
        return x


class FakeClient:
    def __init__(self, fail=False):
        self.calls, self.wait_seconds, self.fail = 0, 0., fail
        self.states = []
    def decide(self, state, allowed, fallback):
        ...
        return {i: 50. for i in allowed}


@pytest.mark.parametrize("name,value", [("mode", []), ("mode", "legacy"), ("mode", None), ("keep_percent", True), ("keep_percent", float("nan")), ("keep_percent", 0), ("keep_percent", 101), ("min_tokens", 32), ("min_tokens", 64.), ("warmup_evaluations", 0), ("update_interval", 0), ("max_calls", True), ("max_calls", 9), ("timeout", float("inf")), ("block_size", True), ("block_size", 129)])
...
@pytest.mark.parametrize("prefix,length,keep", [(0, 135, 50), (17, 135, 50), (137, 130, 75), (17, 192, 1), (3, 128, 100)])
            ...
            for h in range(2):
                ...
                for start in range(0, length, 64):
                    ...
                    for idx in chosen.tolist():
                        allowed[b, h, start:start+64, prefix+idx*64:prefix+min(length, (idx+1)*64)] = True
    reference = F.scaled_dot_product_attention(query.transpose(1,2), key.transpose(1,2), value.transpose(1,2), attn_mask=allowed).transpose(1,2)
...
def test_prefill_exact_cache_and_dense_off_identity(tmp_path):
    ...
    with q.experiment(b,q.Options(mode="fixed",keep_percent=50,min_tokens=64,block_size=64),tmp_path,upstream=UPSTREAM) as run:
        ...
        for cache,(ck,cv) in zip(b.caches,old):
...
def test_real_observations_drive_jev_from_dense_prefill(tmp_path):
    model = QwenImage21Transformer2DModel()
    client = FakeClient()
    with q.experiment(model,q.Options(mode="jev",min_tokens=64,block_size=64,max_calls=2,update_interval=2),tmp_path,upstream=UPSTREAM,client=client) as run:
...
def test_rules_offline_and_client_failure_dense(tmp_path):
    for mode,client in [("rules",None),("jev",FakeClient(fail=True))]:
        model = QwenImage21Transformer2DModel()
        with q.experiment(model,q.Options(mode=mode,min_tokens=64,block_size=64),tmp_path,upstream=UPSTREAM,client=client) as run:
...
def test_cleanup_on_exception_and_cancellation(tmp_path):
    for cancel in (False,True):
        ...
        with pytest.raises((RuntimeError,common.ExperimentCancelled)):
            with q.experiment(model,q.Options(mode="dense"),tmp_path,upstream=UPSTREAM,cancelled=lambda:flag[0]) as run:
                call_prefill(model)
                if cancel:
                    flag[0]=True
...
def fake_service(monkeypatch,tmp_path):
    ...
    monkeypatch.setattr(integration,"WORKER_BLOB",hashlib.sha1(b"blob "+str(len(source)).encode()+b"\0"+source).hexdigest())
...
def test_actual_qwen_service_routing_and_uninstall(monkeypatch,tmp_path):
    ...
    monkeypatch.setenv(q.OPTIONS_ENV,json.dumps({"mode":"fixed"}))
    try:
        integration.install(); integration.install()
        assert service.WORKER.name=="qwen_image21_sparse_worker.py"
        assert service.Studio().start(None,"owner")=="original-job"
        env=service.safe_environment()
        assert "TYPESAFE_API_KEY" not in env
        assert json.loads(env[q.OPTIONS_ENV])["mode"]=="fixed"
    finally:
        integration.uninstall()
    assert service.Studio.start is original_start and service.WORKER==original_worker
...
def test_incompatible_source_blocks_instead_of_fake_benchmark(monkeypatch,tmp_path):
    ...
    try:
        integration.install()
        with pytest.raises(service.QwenImage21Error):
            service.Studio().start(None,"owner")
    finally:
        integration.uninstall()
...
def test_no_launch_option_means_no_hooks(monkeypatch):
    ...
    integration.install()
    assert not integration._RESTORES
...
def test_cloud_key_scoped_to_selected_qwen_worker(monkeypatch,tmp_path):
    ...
    monkeypatch.setattr(integration,"cloud_environment",lambda:{"TYPESAFE_API_KEY":"fake-secret"})
    ...
    try:
        ...
        assert "fake-secret" not in env[q.OPTIONS_ENV]
    finally:
        integration.uninstall()
...
def load_worker(monkeypatch,tmp_path):
    ...
    def run(payload):
        ...
        r["pipe"](use_kv_cache=True,true_cfg_scale=1.)
    ...
    spec=importlib.util.spec_from_file_location("test_qwen21_sparse_worker",ROOT/"tools/qwen_image21_sparse_worker.py")
...
def make_job(tmp_path,name):
    job=tmp_path/name
    job.mkdir()
    (job/"request.json").write_text(json.dumps({"prompt":"private test prompt","seed":7,"width":1024,"height":1024,"steps":2,"precision":"int8","memory_mode":"offload","input_images":[]}),encoding="utf8")
    return job
...
def test_worker_metadata_routing_and_two_job_model_reuse(monkeypatch,tmp_path):
    ...
    monkeypatch.setenv(q.OPTIONS_ENV,json.dumps({"mode":"fixed","keep_percent":50,"block_size":64,"min_tokens":64}))
    results=[]
    ...
    assert results[0]["metadata"]["sparse_experiment"]["log_path"]!=results[1]["metadata"]["sparse_experiment"]["log_path"]
