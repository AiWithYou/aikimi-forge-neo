...
from modules_forge.jev_sparse import common, anima, h3_node, h3_integration as integration
...
@pytest.mark.parametrize("field,value", [("mode", "bogus"), ("keep_percent", True), ("keep_percent", float("nan")), ("keep_percent", 0), ("keep_percent", 101), ("min_tokens", 63), ("min_tokens", 4096.5), ("warmup_evaluations", -1), ("max_calls", True), ("max_calls", 9), ("update_interval", 0), ("timeout", float("inf")), ("timeout", 0.1)])
...
def test_decisions_confidence_and_ids():
    assert common.parse_decisions({"decisions": {"0": {"choice": "50", "confidence": .2}, "1": {"choice": 75, "confidence": .9}}}, {"0": (50, 75, 100), "1": (50, 75, 100)}, 100) == {"0": 100, "1": 75}


@pytest.mark.parametrize("answer", [None, {}, {"decisions": {}}, {"decisions": {0: {"choice": 50, "confidence": .9}}}, {"decisions": {"0": {"choice": True, "confidence": .9}}}, {"decisions": {"0": {"choice": 50, "confidence": True}}}, {"decisions": {"0": {"choice": 1, "confidence": .9}}}, {"decisions": {"0": {"choice": 50, "confidence": float("nan")}}}, {"decisions": {"0": {"choice": 50, "confidence": 1.1}}}])
...
def test_env_explicit_permission_and_least_privilege():
    ...
    result = common.cloud_environment({"AIKIMI_JEV_ALLOW_CLOUD": "1", "TYPESAFE_API_KEY": "fake-key", "GITHUB_TOKEN": "must-not-leak", "HTTP_PROXY": "bad", "PYTHONPATH": "bad", "PATH": "okay"})
...
def test_sdk_timeout_has_bounded_single_attempt(monkeypatch):
    ...
    client = common.JevClient(Path(sys.executable), .5)
...
def test_real_cpu_attention_output_and_inactive_identity():
    ...
    def kernel(q, k, v, keep):
        ...
        return torch.nn.functional.scaled_dot_product_attention(q.transpose(1, 2), k[:, :n].transpose(1, 2), v[:, :n].transpose(1, 2)).transpose(1, 2)
    def original(q, k, v, transformer_options):
        dense_calls.append(True)
        return mod.output_proj(v.flatten(2))
    run = anima.AnimaRun(common.AnimaOptions(mode="fixed", keep_percent=50, min_tokens=64, warmup_evaluations=0), 1, MemoryLog())
...
def test_short_sequence_stays_dense():
    ...
    original = lambda q, k, v, transformer_options: v.flatten(2)
...
def test_nonfinite_anima_observation_falls_back():
    ...
    run.pending[0] = torch.tensor([float("nan"), 0.])
    run.end_evaluation()
    assert run.circuit_open and run.keeps == {"0": 100}


def graph():
    return {"1": {"class_type": "UNETLoader", "inputs": {}}, "5": {"class_type": "MiniMaxH3ImageToVideo", "inputs": {"prompt": "private scene"}}, "7": {"class_type": "KSamplerSelect", "inputs": {"sampler_name": "res_multistep"}}, "8": {"class_type": "BasicScheduler", "inputs": {"steps": 4}}, "9": {"class_type": "BasicGuider", "inputs": {"model": ["15", 0]}}}
...
def test_h3_jev_batches_50_then_49_four_calls():
    ...
    c.initialize()
    for index in range(4):
        c.pending = {i: torch.tensor([1., -1., 2., -1.]) for i in range(50)}
        c.done(index)
...
def test_subprocess_proxy_key_is_scoped(monkeypatch):
    ...
    assert [bool(kwargs["env"].get("TYPESAFE_API_KEY")) for _, kwargs in calls] == [False, False, False, False, True, False]


def test_pack_removal_preserves_unknown_flags():
    parse = lambda args: (integration.PACK, "UnknownPack")
    args = ["main.py", "--listen", "0.0.0.0", "--whitelist-custom-nodes", integration.PACK, "UnknownPack", "--enable-manager"]
...
def test_installer_dry_run_is_offline():
    ...
    result = subprocess.run([sys.executable, str(ROOT / "tools/setup_jev_sparse.py"), "--sdk", "--create-h3-runtime", "--dry-run"], capture_output=True, text=True, check=True)
...
def test_anima_refs_added_after_attach_rejected(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError, match="reference"):
        patched.model_options["model_function_wrapper"](lambda *a: pytest.fail("model executed"), {"input": torch.ones(1), "timestep": torch.ones(1), "c": {}})
    assert run.closed
...
def test_anima_failure_deactivates_context(monkeypatch, tmp_path):
    ...
    with pytest.raises(RuntimeError, match="fake GPU"):
        patched.model_options["model_function_wrapper"](fail, {"input": torch.ones(1), "timestep": torch.ones(1), "c": {}})
...
def fake_comfy(monkeypatch):
    ...
    for name, module in [("comfy", comfy), ("comfy.model_management", mm), ("comfy.patcher_extension", pe), ("comfy_extras", extras), ("comfy_extras.nodes_sparse_attention", sparse)]:
    ...
    class Model:
        ...
        def get_model_object(self, name):
            return types.SimpleNamespace(blocks=self.blocks) if name == "diffusion_model" else types.SimpleNamespace(percent_to_sigma=lambda p: 1-p)
        def clone(self):
            return Model()
        ...
        def __call__(self, model, sigmas, extra, cb, noise, *args):
            ...
            for step in range(4):
                for block in patched.patches.values():
                    x = block({"img": x, "rope_freqs": None, "transformer_options": {}}, {"original_block": lambda a: {"img": a["img"] + 1}})["img"]
                cb(step, x, x, 4)
            return x
    result = patched.wrappers["sample"](Executor(), None, torch.linspace(1, 0, 5), {}, None, torch.zeros(2, 2))
    assert torch.equal(result, torch.full((2, 2), 200.))
...
@pytest.mark.parametrize("sigmas,sampler", [(9, "sample_res_multistep"), (21, "sample_res_multistep"), (5, "sample_euler")])
def test_h3_node_rejects_wrong_actual_sampler(monkeypatch, sigmas, sampler):
    ...
    executor = types.SimpleNamespace(class_obj=types.SimpleNamespace(sampler_function=types.SimpleNamespace(__name__=sampler)))
...
def test_install_pack_atomic_and_reject_user_edits(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(module, "SPARSE_BLOB", integration.git_blob(source))
...
def test_policy_hooks_default_delegation_roundtrip_and_uninstall(monkeypatch):
    ...
    bridge._runtime_arguments_are_allowed = lambda args: set(parse(args)) <= {"OldPack"} and "--enable-manager" not in args
    def start(runtime_root, server_url, log_directory, runtime_profile="fast", wait_seconds=120, initial_readiness=None, acceleration=None):
    ...
    try:
        ...
        assert not bridge._runtime_arguments_are_allowed(["main.py", "--whitelist-custom-nodes", integration.PACK, "UnknownPack"])
        ...
        with pytest.raises(ValueError):
            option.validate_nodes(schema)
        assert bridge._start_runtime_locked(Path("."), "localhost", Path("."), acceleration=Policy()) == "original-start"
...
def test_real_gradio_anima_ui_default_off(monkeypatch):
    ...
    modules.script_callbacks = types.SimpleNamespace(on_before_ui=lambda fn: None, on_after_component=lambda fn: None, on_script_unloaded=lambda fn: None)
    ...
    spec = importlib.util.spec_from_file_location("test_jev_entry", ROOT / "extensions-builtin/jev-sparse-experiments/scripts/jev_sparse_experiments.py")
...
def test_report_preserves_timing_scope_without_speedup(tmp_path):
    ...
    log.finish("completed", timing_scope="sum_instrumented_model_evaluations", model_seconds=12.5, attention_calls={"sparse": 5}, api_calls=0, api_wait_seconds=0)
