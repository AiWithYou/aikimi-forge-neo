...
def _validated_checkpoint_shard(checkpoint_folder, shard):
    """Resolve an index entry without allowing lexical directory traversal.

    Keep symlinks: Hugging Face snapshots legitimately link to sibling blobs.
    The index may choose files beneath the snapshot, never arbitrary paths.
    Validate Windows syntax on every host so a portable index stays safe.
    """
    import ntpath

    if not isinstance(shard, str) or not shard or "\x00" in shard:
        raise ValueError("Checkpoint shard must be a nonempty relative filename")
    if ntpath.splitdrive(shard)[0] or shard.startswith(("/", "\\")) or ":" in shard:
        raise ValueError("Checkpoint shard must be a relative filename")
    root = os.path.abspath(checkpoint_folder)
    candidate = os.path.abspath(os.path.join(root, shard.replace("\\", "/")))
    if os.path.commonpath([root, candidate]) != root or candidate == root:
        raise ValueError("Checkpoint shard escapes its checkpoint directory")
...
def load_checkpoint_in_model(
    ...
    if index_filename is not None:
        ...
        checkpoint_files = [_validated_checkpoint_shard(checkpoint_folder, f) for f in checkpoint_files]
