...
class PipelineProxy:
    ...
    def __call__(self, *args, **kwargs):
        if kwargs.get("use_kv_cache", True) is not True or kwargs.get("true_cfg_scale", 1.) != 1.:
            raise ValueError("Qwen sparse benchmark requires prefix KV caching and true_cfg_scale=1")
        cancelled = lambda: (self.job / "cancel").exists()
        with experiment(self.pipe.transformer, self.options, self.job / "jev-sparse", prompt=self.request["prompt"], cancelled=cancelled) as run:
            self.run = run
            if run is not None:
                digests = []
                ...
                run.log.write("generation_context", seed=self.request["seed"], width=self.request["width"], height=self.request["height"], steps=self.request["steps"], precision=self.request["precision"], memory_mode=self.request["memory_mode"], input_sha256=digests)
            return self.pipe(*args, **kwargs)


def resident_run(payload):
    options = Options.parse(os.environ.get(OPTIONS_ENV, "{}"))
    ...
    try:
        ...
        report = proxies[-1].run.summary if proxies and proxies[-1].run else {"status": "off", "settings": asdict(options)}
