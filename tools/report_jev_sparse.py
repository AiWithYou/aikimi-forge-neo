...
def summarize(path: Path) -> dict:
    ...
    first = records[0]
    if any(not isinstance(row, dict) or row.get("run_id") != first.get("run_id") or row.get("schema") != 1 for row in records):
    ...
    return {"file": str(path), "run_id": first["run_id"], "target": target,
            "mode": settings.get("mode"), "status": "incomplete_log" if incomplete else last.get("status", "incomplete"),
            ...
            "api_calls": last.get("api_calls"), "api_wait_seconds": last.get("api_wait_seconds"),
            "attention_calls": counts, "prompt_sha256": first.get("prompt_sha256"),
...
def main():
    ...
    print(json.dumps({"note": "Measured scopes differ: H3 sampling only; Anima summed instrumented model evaluations. Match models, prompts, media, seeds, sizes and runtime before comparing. No automatic speedup claim.", "runs": reports}, ensure_ascii=False, indent=2, allow_nan=False))
