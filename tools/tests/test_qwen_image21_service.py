...
class QwenUiTests(unittest.TestCase):
    ...
    def test_reorder_remove_and_owner_bound_result_reuse(self):
        ...
        self.assertEqual(self.ui.reference_paths(update.constructor_args["value"]), ["second.png", "first.png"])
        self.assertEqual(selected, 0)
        update, selected, controls = self.ui.remove_reference(update.constructor_args["value"], selected)
        self.assertTrue(controls["visible"])
        self.assertEqual(self.ui.reference_paths(update.constructor_args["value"]), ["first.png"])
        ...
        with patch.object(self.ui.STUDIO, "artifact", return_value=Path("output.png")) as artifact:
            update, selected, controls = self.ui.use_result("job-id", update.constructor_args["value"], request)
        self.assertTrue(controls["visible"])
        artifact.assert_called_once_with("job-id", "user:session")
        self.assertEqual(self.ui.reference_paths(update.constructor_args["value"]), ["first.png", "output.png"])
